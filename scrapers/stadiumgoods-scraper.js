  ...
  getLink: async function (shoe, callback) {
    try {
      ...
      if (response.body.data.configurableProducts.edges[0]) {
        shoe.resellLinks.stadiumGoods =
          response.body.data.configurableProducts.edges[0].node.pdpUrl;
        if (
          response.body.data.configurableProducts.edges[0].node.lowestPrice
            .__typename == "DiscountedPrice"
        ) {
          shoe.lowestResellPrice.stadiumGoods = Number(
            response.body.data.configurableProducts.edges[0].node.lowestPrice.originalValue.formattedValue.replace(
              /[^0-9.-]+/g,
              ""
            )
          );
          ...
          shoe.lowestResellPrice.stadiumGoods = Number(
            response.body.data.configurableProducts.edges[0].node.lowestPrice.value.formattedValue.replace(
              /[^0-9.-]+/g,
              ""
            )
          );
        ...
        callback(
          new Error(
            "Product '" + shoe.styleID + "' not found on Stadium Goods'"
          )
        );
      }
    } catch (error) {
  ...
  getPrices: function (shoe, callback) {
    if (!shoe.resellLinks.stadiumGoods) {
      ...
      request.post(options, function getPriceMap(error, response, data) {
        if (error) {
          let err = new Error(
            "Could not connect to Stadium Goods while searching '",
            shoe.styleID,
            "' Error: ",
            error
          );
...
export default stadiumGoodsScraper;
