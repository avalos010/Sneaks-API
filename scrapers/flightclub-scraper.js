...
const flightClubScraper = {
  getLink: async function (shoe, callback) {
    var options = {
      ...
      body:
        '{"requests":[{"indexName":"product_variants_v2_flight_club","params":"query=' +
        shoe.styleID +
        '&hitsPerPage=1&maxValuesPerFacet=1&filters=&facets=%5B%22lowest_price_cents_usd%22%5D&tagFilters="}]}',
      headers: {
        "User-Agent":
    ...
    request.post(options, function (error, response, data) {
      if (error) {
        let err = new Error(
          "Could not connect to Flight Club while searching '" +
            shoe.styleID +
            "' Error: ",
          error
        );
        ...
        if (json.results[0].hits[0]) {
          ...
          shoe.resellLinks.flightClub =
            "https://www.flightclub.com/" + json.results[0].hits[0].slug;
      ...
      try {
        ...
        try {
          const response = await got.post(
            "https://www.flightclub.com/graphql",
            {
              headers: {
                "User-Agent":
                  "Mozilla/5.0 (Windows NT 6.2; rv:20.0) Gecko/20121202 Firefox/20.0",
          ...
          for (
            var i = 0;
            i < json.data.getProductTemplate.newSizes.length;
            i++
          ) {
            priceMap[json.data.getProductTemplate.newSizes[i].size.display] =
              json.data.getProductTemplate.newSizes[i].lowestPriceOption.price
                .value / 100;
          }
        ...
        let err = new Error(
          "Could not connect to Flight Club while grabbing token '",
          "' Error: ",
          error
        );
        console.log(err);
...
export default flightClubScraper;
